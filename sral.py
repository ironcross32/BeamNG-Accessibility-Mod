...
class Sral:
    ...
    def speak_ssml(self, text, interrupt=True):
        return self.lib.SRAL_SpeakSsml(
            c_char_p(text.encode("utf-8")), c_bool(interrupt)
        )
    ...
    def set_engine_parameter(self, engine, param, value):
        return self.lib.SRAL_SetEngineParameter(
            c_int(engine), c_int(param), c_int(value)
        )
    ...
    def speak_ex(self, engine, text, interrupt=True):
        return self.lib.SRAL_SpeakEx(
            c_int(engine), c_char_p(text.encode("utf-8")), c_bool(interrupt)
        )

    def speak_ssml_ex(self, engine, text, interrupt=True):
        return self.lib.SRAL_SpeakSsmlEx(
            c_int(engine), c_char_p(text.encode("utf-8")), c_bool(interrupt)
        )
